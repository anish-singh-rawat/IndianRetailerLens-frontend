...
import {
    ...
    FiPackage, FiCalendar, FiPrinter, FiTag, FiCheckSquare, FiChevronDown,
    FiPercent, FiDollarSign, FiBarChart2, FiGrid, FiSliders, FiLayout, FiHash, FiAlignLeft, FiHome, FiBox, FiLayers, FiAward, FiColumns, FiAlignRight, FiMaximize2,

  FiMove, FiArrowUp, FiArrowDown,FiEyeOff, FiEye,FiSave
} from "react-icons/fi";
...
function ProductKeywordInput({ value, onChange }) {
    ...
    const fetchSuggestions = async (q) => {
        ...
        try {
            setSearching(true);
            const res = await api.get("/product/suggestion", { params: { q } });
...
function InventoryTable({ fromDate, setFromDate, toDate, setToDate, keyword, setKeyword, triggerSearch, setTriggerSearch }) {
    ...
    const fetchProducts = async (pageNumber = 1, append = false) => {
        try {
            ...
            const res = await api.get("/product", { params: { page: pageNumber, limit: FETCH_LIMIT } });
            if (res.data.success) {
    ...
    const handleDeleteAllProducts = async () => {
        ...
        const { value: typed } = await swalInstance.fire({
            title: "DELETE ALL INVENTORY",
            html: `
            <p style="color:#6b7280;font-size:13px;margin-bottom:12px">
                This will permanently remove <strong>every product</strong> from your store.
        ...
        `,
            ...
            didOpen: () => {
                ...
                input.addEventListener("input", (e) => {
                    ...
                    confirmButton.style.cursor = isValid
                        ? "pointer"
                        : "not-allowed";
                });
            },
    ...
    const handleInventorySubmit = async (e) => {
        ...
        try {
            const res = await api.post("/product/add/inventory", { items: inventoryRows });
            if (res.data.success) {
    ...
    const searchProducts = async () => {
        ...
        try {
            ...
            const res = await api.post("/product/search", { startDate: fromDate || undefined, endDate: toDate || undefined, keyword: keyword || undefined });
        ...
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
            ...
            {/* DELETE ALL PRODUCT */}
                {isAdmin && (
                    <button
                        ...
                        className="flex items-center gap-1.5 px-3 py-1.5 bg-red-600 hover:bg-red-700 text-white text-xs font-semibold rounded-xl transition shadow-sm"
                    >
            ...
            {openInventoryModal && (
                <InventoryModal open={openInventoryModal}
                    ...
                    settings={settings} vendors={vendors}
...
function EditProductModal({ product, settings, onClose }) {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            const res = await api.put(`/product`, dataToSend, { headers: { "Content-Type": "multipart/form-data" } });
...
const InventoryHistoryModal = ({ product, onClose }) => {
    ...
    const fetchInventoryHistory = async () => {
        try {
            setLoading(true);
            const res = await api.get(`/product/inventory/${product._id}`);
...
// const STORAGE_KEY = "printModalSettings_v6";
...
// // All drag-drop zones for text fields
...
// // imageSide controls which physical column the barcode/QR sits in.
// // "hidden" = the image element is not rendered, BUT all text fields still appear.
// const IMAGE_SIDE_OPTIONS = [
//   { value: "left",   label: "Image Left",   icon: FiChevronLeft,  desc: "Barcode/QR on left — text on right." },
//   { value: "right",  label: "Image Right",  icon: FiChevronRight, desc: "Text on left — Barcode/QR on right." },
//   { value: "hidden", label: "Hide Image",   icon: FiEyeOff,       desc: "Image hidden — text fields still print." },
// ];
...
// /** Normalise padding: accepts old flat-number OR 4-sided object */
...
//   const map = {
//     loading:  { cls: "bg-blue-50 text-blue-500 border-blue-100",   label: "Looking up…" },
...
//       {label && (
//         <label className={`${sm ? "text-[8px]" : "text-[9px]"} font-bold text-gray-400 uppercase tracking-widest flex items-center gap-1`}>
//           <FiHash size={sm ? 8 : 9} /> {label}
//         </label>
//       )}
//       <div className="flex items-center gap-1">
//         <button type="button" onClick={() => onChange(Math.max(1, parsed - 1))}
//           className={`${sm ? "w-6 h-6 text-[10px]" : "w-7 h-7 text-xs"} rounded-lg border border-gray-200 text-gray-500 hover:bg-gray-50 font-bold flex items-center justify-center`}>−</button>
//         <input type="number" min={1} value={value} onChange={e => onChange(e.target.value)}
//           className={`${sm ? "w-10 text-[10px] py-0.5" : "w-12 text-xs py-1"} text-center font-semibold rounded-lg border border-gray-200 outline-none focus:border-orange-300 text-gray-700`} />
...
//               ${active ? "border-orange-300 bg-orange-50 text-orange-600" : "border-gray-200 bg-white text-gray-500 hover:border-orange-200 hover:text-orange-500"}`}>
//             {s.label}
//           </button>
//         );
//       })}
...
//         className={`px-3 py-1.5 rounded-xl border text-xs font-semibold transition
//           ${customOpen || !isPreset ? "border-orange-300 bg-orange-50 text-orange-600" : "border-gray-200 bg-white text-gray-500 hover:border-orange-200 hover:text-orange-500"}`}>
//         Custom
//       </button>
//       {(customOpen || !isPreset) && (
//         <div className="flex items-center gap-1">
//           <input type="number" value={width} onChange={e => onChange({ width: +e.target.value, height })}
//             className="w-14 text-center text-xs rounded-lg border border-gray-200 py-1 outline-none focus:border-orange-300" placeholder="W" />
//           <span className="text-gray-400 text-xs">×</span>
//           <input type="number" value={height} onChange={e => onChange({ width, height: +e.target.value })}
//             className="w-14 text-center text-xs rounded-lg border border-gray-200 py-1 outline-none focus:border-orange-300" placeholder="H" />
...
//   return (
//     <div
//       draggable onDragStart={() => onDragStart(fieldKey)} onDragEnd={onDragEnd}
...
//         ${dragging === fieldKey ? "opacity-40 scale-95" : "hover:border-orange-200 hover:shadow-sm"}`}
//     >
...
//             className="p-0.5 rounded hover:bg-orange-50 text-gray-300 hover:text-orange-400 disabled:opacity-20 transition"><FiArrowUp size={9} /></button>
//           <button type="button" onClick={() => onReorder(zone, fieldKey, 1)} disabled={idx === arr.length - 1}
//             className="p-0.5 rounded hover:bg-orange-50 text-gray-300 hover:text-orange-400 disabled:opacity-20 transition"><FiArrowDown size={9} /></button>
//         </div>
//       )}
//       <button type="button" onClick={() => onHide(fieldKey)} title="Hide"
//         className="p-1 rounded-lg hover:bg-red-50 text-gray-300 hover:text-red-400 transition"><FiX size={10} /></button>
//     </div>
//   );
// }
...
//         ${overZone === zoneId ? "border-orange-400 bg-orange-50" : "border-gray-200 bg-gray-50"}`}>
...
//         </DropZone>
//         <DropZone {...dzProps("hidden")} label="Hidden" icon={<FiEyeOff size={11} />} hint="not printed">
...
//               className="text-[10px] px-2 py-1 rounded-lg border border-dashed border-orange-300 text-orange-400 hover:bg-orange-50 transition">
//               + {f.label}
//             </button>
//           ))}
//         </div>
...
//             className="w-14 text-center text-[11px] rounded-lg border border-gray-200 py-1 outline-none focus:border-orange-300 text-gray-700" />
...
//             <span className="text-[9px] text-gray-400">mm</span>
//           </div>
//           <div className="w-12 h-8 border-2 border-dashed border-orange-200 rounded-md bg-orange-50 flex items-center justify-center">
//             <span className="text-[8px] text-orange-300 font-bold">zone</span>
//           </div>
//           <div className="flex flex-col items-center gap-0.5">
...
//         </div>
//       </div>
//       <div className="mt-1.5 text-center text-[9px] text-orange-500 font-mono">
...
//                 ${isActive ? "border-orange-400 bg-orange-50 text-orange-600" : "border-gray-200 bg-white text-gray-500 hover:border-orange-200 hover:text-orange-500"}`}>
...
//           {value === "hidden" && <span className="text-amber-500 font-semibold">⚠ </span>}
//           {active.desc}
//         </p>
//       )}
//     </div>
...
//       onChange({ ...typo, fonts: { ...typo.fonts, [key]: val } });
//     } else {
...
//           <SliderRow label="Image / code size"  min={30} max={100} step={1}   value={typo.imgPct}      unit="%"  onChange={v => set("imgPct", v)} />
...
//           {badge && <span className="text-[9px] px-2 py-0.5 rounded-full bg-orange-100 text-orange-600 font-semibold">{badge}</span>}
//         </div>
...
//   const imageHidden = imageSide === "hidden";   // image element hidden, text still shows

//   const W = Math.round(labelSize.width  * MM_TO_PX);
//   const H = Math.round(labelSize.height * MM_TO_PX);

//   // When image is hidden we use two columns only if "right" zone has fields
//   const hasRightZone = (layout.right || []).length > 0;
//   // Split percent: image takes leftZonePct of width; when hidden the "image column" shrinks to 0
//   const lzPct = imageHidden
//     ? (hasRightZone ? 0 : 100)       // image hidden: right zone gets all space if it exists
//     : (hasRightZone ? typo.leftZonePct : 100);
//   const lzW    = Math.round(W * (imageHidden && hasRightZone ? 0 : lzPct) / 100);
//   const fScale = Math.max(0.5, Math.min(1.4, Math.max(lzW, W - lzW) / (50 * MM_TO_PX)));
...
//   useEffect(() => {
//     if (printType !== "barcode" || !barcodeCanvasRef.current || !value || imageHidden) return;
//     try {
...
//       });
//     } catch {}
//   }, [value, printType, imgW, imgH, imageHidden]);

//   useEffect(() => {
//     if (printType !== "qr" || !value || imageHidden) { setQrUrl(null); return; }
//     QRCode.toDataURL(value, { width: Math.max(40, imgH * 2), margin: 1 })
//       .then(setQrUrl).catch(() => setQrUrl(null));
//   }, [value, printType, imgH, imageHidden]);
...
//   // ── When image is hidden: collect ALL visible text zones into one full-width column ──
//   if (imageHidden) {
//     // Gather fields from all non-hidden zones in a sensible order:
//     // leftAbove, leftBelow (were in image column), right (text column)
//     // leftOfImage / rightOfImage also shown since image is gone
//     const allTextZones = ["leftAbove", "leftOfImage", "rightOfImage", "leftBelow", "right"];
//     return (
//       <div style={{ width: W, height: H, display: "flex", border: "0.5px solid #d1d5db", borderRadius: 2, background: "#fff", overflow: "hidden", flexShrink: 0 }}>
//         <div style={{
//           width: W, height: "100%",
...
//         }}>
//           {allTextZones.map(z => renderZoneFields(z))}
//         </div>
//       </div>
//     );
//   }

//   // ── Normal layout: image on left or right ─────────────────────────────────
//   const imgEl = (
//     printType === "barcode"
...
//   // imageZone = the column that contains the barcode image + leftAbove/leftBelow fields
//   const imageZoneEl = (
//     <div style={{
...
//       gap: `${gapPx}px`, overflow: "hidden",
//       borderRight: imageSide === "left" && hasRightZone ? "0.5px solid #e5e7eb" : "none",
//       borderLeft:  imageSide === "right" && hasRightZone ? "0.5px solid #e5e7eb" : "none",
//     }}>
...
//   // textZone = the column that contains "right" zone fields
//   const textZoneEl = hasRightZone ? (
//     <div style={{
...
//       {imageSide === "left"  && imageZoneEl}
//       {imageSide === "left"  && textZoneEl}
//       {imageSide === "right" && textZoneEl}
//       {imageSide === "right" && imageZoneEl}
//     </div>
//   );
// }
...
//     const imageSide = typo.imageSide || "left";
//     const imgHidden = imageSide === "hidden";

//     const dataUrls = await Promise.all(labels.map(async l => {
//       if (imgHidden) return null;
//       if (!isQR) {
//         const canvas = document.createElement("canvas");
//         try {
//           JsBarcode(canvas, l.barcodeValue, { format: "CODE128", width: 2, height: 60, displayValue: false, margin: 4, background: "#ffffff", lineColor: "#000000" });
//           return canvas.toDataURL("image/png");
//         } catch { return null; }
//       } else {
//         try { return await QRCode.toDataURL(l.barcodeValue, { width: 160, margin: 2, color: { dark: "#000000", light: "#ffffff" } }); }
//         catch { return null; }
//       }
//     }));
...
//       // ── Image hidden: merge all text zones into one full-width column ──────
//       if (imgHidden) {
//         const allText = [
//           ...label.leftAbove,
//           ...label.leftOfImage,
...
//           ...label.right,
//         ];
//         return `<div style="width:${CW}mm;height:${PH}mm;flex-shrink:0;display:flex;flex-direction:row;align-items:stretch;overflow:hidden;">
//           <div style="flex:1;min-width:0;height:100%;display:flex;flex-direction:column;align-items:flex-start;justify-content:center;padding:${rpad.top}mm ${rpad.right}mm ${rpad.bottom}mm ${rpad.left}mm;gap:${typo.gap}mm;overflow:hidden;">
//             ${allText.map(fieldDiv).join("")}
//           </div>
//         </div>`;
//       }

//       // ── Normal: image on left or right ────────────────────────────────────
//       const loiHtml = hasLOI ? `<div style="flex-shrink:0;max-width:35%;display:flex;flex-direction:column;align-items:flex-end;justify-content:center;gap:${typo.gap}mm;overflow:hidden;">${label.leftOfImage.map(fieldDivR).join("")}</div>` : "";
//       const roiHtml = hasROI ? `<div style="flex-shrink:0;max-width:35%;display:flex;flex-direction:column;align-items:flex-start;justify-content:center;gap:${typo.gap}mm;overflow:hidden;">${label.rightOfImage.map(fieldDiv).join("")}</div>` : "";
//       const imgHtml = imgSrc  ? `<img src="${imgSrc}" style="width:${typo.imgPct}%;height:auto;object-fit:contain;display:block;flex-shrink:1;min-height:0;${isQR ? "image-rendering:pixelated;max-height:70%;" : "max-height:65%;"}" alt="">` : "";
...
//       const imgColStyle = `width:${LZP}%;flex-shrink:0;height:100%;display:flex;flex-direction:column;align-items:center;justify-content:center;padding:${lpad.top}mm ${lpad.right}mm ${lpad.bottom}mm ${lpad.left}mm;gap:${typo.gap}mm;overflow:hidden;`;
//       const txtColStyle = `flex:1;min-width:0;height:100%;display:flex;flex-direction:column;align-items:flex-start;justify-content:center;padding:${rpad.top}mm ${rpad.right}mm ${rpad.bottom}mm ${rpad.left}mm;gap:${typo.gap}mm;overflow:hidden;`;

//       const imgColHtml = `<div style="${imgColStyle}${imageSide === "right" ? "border-left:0.2mm solid #eee;" : "border-right:0.2mm solid #eee;"}">${label.leftAbove.map(fieldDiv).join("")}${imgRow}${label.leftBelow.map(fieldDiv).join("")}</div>`;
//       const txtColHtml = hasRight ? `<div style="${txtColStyle}">${label.right.map(fieldDiv).join("")}</div>` : "";
...
//         const idx = i + offset;
//         return idx < labels.length ? renderCell(labels[idx], dataUrls[idx], false) : renderCell(null, null, true);
//       });
...
//   // ─── Render ───────────────────────────────────────────────────────────────
//   const inputCls = "w-full text-xs rounded-xl border border-gray-200 px-3 py-2 outline-none focus:border-orange-300 focus:bg-white bg-white transition placeholder:text-gray-300 text-gray-700";
...
//           <div className="p-2 bg-orange-50 rounded-xl"><FiPrinter size={16} className="text-orange-500" /></div>
...
//               className="flex items-center gap-1.5 px-3 py-1.5 rounded-xl border border-gray-200 bg-white text-xs font-semibold text-gray-500 hover:border-orange-300 hover:text-orange-500 transition">
//               <FiRefreshCw size={12} /> Reset
//             </button>
//             <button type="button" onClick={onClose} className="p-2 rounded-xl hover:bg-gray-100 text-gray-400 transition"><FiX size={16} /></button>
//           </div>
//         </div>
...
//                 className={`px-3 py-1.5 rounded-xl border text-xs font-semibold transition
//                   ${nup === n && !isCustomN ? "border-orange-300 bg-orange-50 text-orange-600" : "border-gray-200 bg-white text-gray-500 hover:border-orange-200 hover:text-orange-500"}`}>
//                 {n}-Up
//               </button>
//             ))}
//             <span className="w-px h-5 bg-gray-200" />
//             <div className={`flex items-center gap-1.5 px-3 py-1.5 rounded-xl border text-xs font-semibold transition ${isCustomN ? "border-orange-300 bg-orange-50 text-orange-600" : "border-gray-200 bg-white text-gray-500"}`}>
//               <FiSliders size={11} className={isCustomN ? "text-orange-400" : "text-gray-400"} />
...
//             </div>
//             {nup > 1 && (
//               <span className="flex items-center gap-1 text-[9px] bg-orange-500 text-white font-bold px-2 py-0.5 rounded-full">
...
//           {/* Global Quantity */}
//           <div className="flex items-center gap-4 bg-orange-50 border border-orange-100 rounded-xl px-4 py-3 flex-wrap">
//             <div className="flex items-center gap-2">
//               <FiHash size={13} className="text-orange-400" />
//               <span className="text-xs font-semibold text-gray-700">Global Quantity</span>
//             </div>
//             <label className="flex items-center gap-2 cursor-pointer">
//               <div onClick={() => setUseGlobalQty(v => !v)}
//                 className={`relative w-9 h-5 rounded-full transition-colors cursor-pointer ${useGlobalQty ? "bg-orange-500" : "bg-gray-300"}`}>
...
//           {/* Typography & Spacing (includes image side toggle) */}
...
//                       ${on ? "border-orange-300 bg-orange-50 text-orange-600" : "border-gray-200 bg-white text-gray-400 hover:border-gray-300"}`}>
...
//                   className="text-xs rounded-lg border border-orange-200 px-2 py-1 outline-none focus:border-orange-400 text-gray-700 w-40" />
//               </div>
//             )}
//           </Section>
...
//             <div className="flex items-center gap-2">
//               <FiLayout size={13} className="text-orange-400" />
//               <span className="text-xs font-semibold text-gray-600">Label layout</span>
//               <span className="text-[10px] text-gray-400">
//                 Above: {(layout.leftAbove || []).length} · LeftImg: {(layout.leftOfImage || []).length} · RightImg: {(layout.rightOfImage || []).length} · Below: {(layout.leftBelow || []).length} · Right: {(layout.right || []).length}
//               </span>
//             </div>
//             <button type="button" onClick={() => setShowDesigner(v => !v)}
//               className={`flex items-center gap-1.5 px-3 py-1.5 rounded-xl border text-xs font-semibold transition
//                 ${showDesigner ? "border-orange-300 bg-orange-50 text-orange-600" : "border-gray-200 bg-white text-gray-500 hover:border-orange-200 hover:text-orange-500"}`}>
...
//                 <FiMove size={12} className="text-orange-400" />
...
//               <FiColumns size={13} className="text-orange-400 flex-shrink-0" />
//               <p className="text-xs text-orange-700">
//                 <strong>{nup}-Up on</strong> — {nup} labels per strip, each cell <strong>~{Math.round(labelSize.width / nup)}mm</strong> wide.
//               </p>
//             </div>
//           )}
//           <div className="flex items-center gap-2 bg-blue-50 border border-blue-100 rounded-xl px-4 py-2.5">
//             <FiInfo size={13} className="text-blue-400 flex-shrink-0" />
...
//                   {nup > 1 && <span className="text-orange-600"> → {totalStrips} strip{totalStrips !== 1 ? "s" : ""} ({nup}-up)</span>}
//                   {" "}will print.
//                 </span>
//               )}
//             </p>
...
//                     <div className="flex items-center gap-2">
//                       <FiHash size={11} className="text-orange-400" />
...
//                     </div>
//                     {rows.length > 1 && (
//                       <button type="button" onClick={() => removeRow(row.id)} className="p-1.5 rounded-lg hover:bg-red-50 text-red-400 transition">
//                         <FiTrash2 size={13} />
//                       </button>
//                     )}
//                   </div>
...
//                           rows={2}
//                           className="w-full text-xs rounded-xl border border-gray-200 px-3 py-2 outline-none resize-none leading-relaxed transition placeholder:text-gray-300 text-gray-700 focus:border-orange-300 focus:bg-white bg-white" />
//                       </div>
//                     )}
...
//                           {(typo.imageSide || "left") === "hidden" && (
//                             <span className="ml-2 text-amber-400 normal-case font-normal">Image hidden — text-only</span>
//                           )}
//                         </div>
//                         <div className="flex items-center gap-3">
//                           <LivePreviewLabel
...
//                               {useGlobalQty && <span className="text-[9px] text-orange-400 ml-1">(global)</span>}
//                             </span>
//                           )}
//                         </div>
//                       </div>
...
//                           label={useGlobalQty ? "Qty (overridden)" : "Qty"}
//                         />
//                       </div>
//                     </div>
//                   </div>
...
//             <FiPrinter size={12} className="text-orange-400" />
//             {validRows.length} code{validRows.length !== 1 ? "s" : ""} · {totalLabels} label{totalLabels !== 1 ? "s" : ""}
//             {nup > 1 && <span className="text-orange-500 font-semibold"> · {totalStrips} strip{totalStrips !== 1 ? "s" : ""} ({nup}-up)</span>}
//           </div>
//           <button type="button" onClick={onClose}
...
//             {nup > 1 && <span className="text-orange-200 text-[9px] font-normal ml-0.5">{nup}-up</span>}
//           </button>
//         </div>
  ...
  return (
    ...
    >
      ...
      {zone !== "hidden" && (
        <div className="flex flex-col gap-0.5 ml-1">
          <button type="button" onClick={() => onReorder(zone, fieldKey, -1)} disabled={idx === 0}
            className="p-0.5 rounded hover:bg-orange-50 text-gray-300 hover:text-orange-400 disabled:opacity-20 transition">
            <FiArrowUp size={9} />
          </button>
          <button type="button" onClick={() => onReorder(zone, fieldKey, 1)} disabled={idx === arr.length - 1}
            className="p-0.5 rounded hover:bg-orange-50 text-gray-300 hover:text-orange-400 disabled:opacity-20 transition">
      ...
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-4xl flex flex-col max-h-[94vh]">
        ...
        <div className="flex-1 min-h-0 overflow-y-auto p-5 space-y-4">
          ...
          <div className="flex items-center gap-4 bg-orange-50 border border-orange-100 rounded-xl px-4 py-3 flex-wrap">
            ...
            <label className="flex items-center gap-2 cursor-pointer">
              <div onClick={() => setUseGlobalQty(v => !v)}
                className={`relative w-9 h-5 rounded-full transition-colors ${useGlobalQty ? "bg-orange-500" : "bg-gray-300"}`}>
                <div className={`absolute top-0.5 left-0.5 w-4 h-4 bg-white rounded-full shadow transition-transform ${useGlobalQty ? "translate-x-4" : ""}`} />
              </div>
              <span className="text-xs text-gray-500">{useGlobalQty ? "Override all rows" : "Use per-row qty"}</span>
            </label>
